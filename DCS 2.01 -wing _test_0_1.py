...
if starting:
    ...
    # -------------------------- 新增：回中优化参数 --------------------------
    global center_delay_frame, current_delay_frame  # 回中延迟帧数（控制延迟时间）
    global stick_slow_down_ratio  # 鼠标摇杆减速倍率（0.1~1.0，越小越慢）
    stick_slow_down_ratio = 0.3  # 减慢70%（可调整，0.5=慢50%，0.3=慢70%）
    center_delay_frame = 10  # 回中延迟帧数（60帧=1秒，10帧≈0.17秒）
    current_delay_frame = 0  # 当前延迟计数（初始为0）

    # 新增：自定义回中速度参数（0.005~0.1，数值越小回中越慢，越大越快）
    global center_speed_x, center_speed_y
    center_speed_x = 0.03  # X轴（滚转）回中速度（默认0.01，原固定值0.02）
    center_speed_y = 0.024  # Y轴（俯仰）回中速度（默认0.008，原固定值0.01）
    # -------------------------------------------------------------------------
...
# 合并版：鼠标/键盘操作后自动回中（自定义速度+无冲突）
if (Re_vjoyaxisx or Re_vjoyaxisy) and not trimp_vjoyaxis_mode:
    # 触发条件：无键盘控制（X/Y轴都没按） AND 鼠标停止移动（X/Y轴都不动）
    no_key_control = not (key_vjoyaxisX or key_vjoyaxisY)
    mouse_stopped = abs(mouse.deltaX) <= 1.5 and abs(mouse.deltaY) <= 1.5
    
    if no_key_control and mouse_stopped:
        current_delay_frame += 1  # 延迟计数累加
        if current_delay_frame >= center_delay_frame:
            # ---------------- X轴回中（自定义速度）----------------
            if Re_vjoyaxisx:
                # 死区吸附回中
                if abs(vjoyaxisX - Trimp_centerX) <= 50:
                    vjoyaxisX = Trimp_centerX
                    vx_max = Trimp_centerX
                    vx_min = Trimp_centerX
                else:
                    # 更新X轴最大/最小值，确保回中平滑
                    if vjoyaxisX >= vx_max:
                        vx_max = vjoyaxisX
                    elif vjoyaxisX <= vx_min:
                        vx_min = vjoyaxisX
                    # 应用自定义X轴回中速度
                    if vjoyaxisX > Trimp_centerX:
                        vjoyaxisX -= (vjoyaxisX - Trimp_centerX) * center_speed_x
                        vx_max -= (vjoyaxisX - Trimp_centerX) * center_speed_x
                    elif vjoyaxisX < Trimp_centerX:
                        vjoyaxisX -= (vjoyaxisX - Trimp_centerX) * center_speed_x
                        vx_min -= (vjoyaxisX - Trimp_centerX) * center_speed_x
            
            # ---------------- Y轴回中（自定义速度）----------------
            if Re_vjoyaxisy:
                # 死区吸附回中
                if abs(vjoyaxisY - Trimp_centerY) <= 50:
                    vjoyaxisY = Trimp_centerY
                    vy_max = Trimp_centerY
                    vy_min = Trimp_centerY
                else:
                    # 更新Y轴最大/最小值，确保回中平滑
                    if vjoyaxisY >= vy_max:
                        vy_max = vjoyaxisY
                    elif vjoyaxisY <= vy_min:
                        vy_min = vjoyaxisY
                    # 应用自定义Y轴回中速度
                    if vjoyaxisY > Trimp_centerY:
                        vjoyaxisY -= (vjoyaxisY - Trimp_centerY) * center_speed_y
                        vy_max -= (vjoyaxisY - Trimp_centerY) * center_speed_y
                    elif vjoyaxisY < Trimp_centerY:
                        vjoyaxisY -= (vjoyaxisY - Trimp_centerY) * center_speed_y
                        vy_min -= (vjoyaxisY - Trimp_centerY) * center_speed_y
    else:
        current_delay_frame = 0  # 有鼠标/键盘操作时，重置延迟计数
# 键盘控制
    ...
    if(vjoyaxis_x):
        if (Frontview_tracking):
            if (Rearview_tracking):
                if m_redu0x > 0:
                    # 应用减速倍率
                    vjoyaxisX -= (mouse.deltaX * stick_vx_sensRear * stick_base_sens * 0.48 * global_sens * stick_slow_down_ratio) / center_redux
                else:
                    vjoyaxisX -= (mouse.deltaX * stick_vx_sensRear * stick_base_sens * 0.48 * global_sens * stick_slow_down_ratio) * center_redux
                ...
                if m_redu0x > 0:
                    # 应用减速倍率
                    vjoyaxisX += (mouse.deltaX * stick_vx_sensFront * stick_base_sens * 0.48 * global_sens * stick_slow_down_ratio) / center_redux
                else:
                    vjoyaxisX += (mouse.deltaX * stick_vx_sensFront * stick_base_sens * 0.48 * global_sens * stick_slow_down_ratio) * center_redux
            ...
            if m_redu0x > 0:
                vjoyaxisX += (mouse.deltaX * stick_vx_sens * stick_base_sens * 30 * speed_multiX * global_sens * stick_slow_down_ratio) / center_redux
            else:
                vjoyaxisX += (mouse.deltaX * stick_vx_sens * stick_base_sens * 30 * speed_multiX * global_sens * stick_slow_down_ratio) * center_redux
        ...
        if (Frontview_tracking):
            if vjoyaxisY >= Trimp_centerY:
                if key_Button4:
                    if m_redu0y > 0:
                        # 应用减速倍率
                        vjoyaxisY -= (mouse.deltaY * stick_vy_sensRear * stick_base_sens * 0.5 * global_sens * stick_slow_down_ratio) / center_reduy
                    else:
                        vjoyaxisY -= (mouse.deltaY * stick_vy_sensRear * stick_base_sens * 0.5 * global_sens * stick_slow_down_ratio) * center_reduy
                else:
                    if m_redu0y > 0:
                        # 应用减速倍率
                        vjoyaxisY -= (mouse.deltaY * stick_vy_sensFront * stick_base_sens * 0.5 * global_sens * stick_slow_down_ratio) / center_reduy
                    else:
                        vjoyaxisY -= (mouse.deltaY * stick_vy_sensFront * stick_base_sens * 0.5 * global_sens * stick_slow_down_ratio) * center_reduy
            else:
                vjoyaxisY = Trimp_centerY
            ...
            if m_redu0y > 0:
                if vjoyaxisY >= Trimp_centerY:
                    vjoyaxisY += (mouse.deltaY * stick_vy_sens * stick_base_sens * 14.4 * speed_multiY * global_sens * stick_slow_down_ratio) / center_reduy
                else:
                    vjoyaxisY += (mouse.deltaY * stick_vy_sens * stick_base_sens * 14.4 * speed_multiY * global_sens * stick_slow_down_ratio) / center_reduy
            else:
                if vjoyaxisY >= Trimp_centerY:
